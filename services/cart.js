...
function createOrder(flag, t_id, user, _dt, merchant, _arr){
  ...
  if (_arr.length > 0) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (_arr[i].productType == GROUP_PURCHASE) { //普通团购
        ...
        c_list.push(_arr[i]);
      } else if (_arr[i].productType ==  B2C_TYPE) {// 电商
        b_list.push(_arr[i]);
      }
    }
  }
  ...
  var bOrder = getB2COrder(flag,b_list,user);
  if(mOrder != null) {
  ...
  if (bOrder != null) {
    totalPay += bOrder.totalPay;
    needPay += bOrder.needPay;
  }
   return {
     totalPay: totalPay,
     needPay: needPay,
...
//获取电商订单
function getB2COrder(flag, _arr, user) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (flag == 0) {
      g = {
        ...
        "comments": _arr[i].comments,
        "b2cId": _arr[i].b2cId,
        "specs": _arr[i].specs,
        "specsStr": _arr[i].specsStr,
      };
    }
    goodsList.push(g);
  }
  //console.log("getB2COrder---------------totalPay " + totalPay + "  needPay " + needPay  );
  b2cOrder.totalPay = totalPay;
  b2cOrder.needPay = needPay;
  b2cOrder.buyNum = buyNum;
  b2cOrder.goodsList = goodsList;
